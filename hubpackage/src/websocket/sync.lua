...
--  local bytes = 3
  ...
  while true do
    local chunk,err = self:sock_receive(math.floor(bytes))
    if err then
...
local close = function(self,code,reason)
  ...
  if n == #encoded then
    ...
    was_clean = true    --*****
    --[[  ** Let channel handler receive response **
  ...
  --self:sock_close()     -- let channel handler do it
  if self.on_close then
    print('WS Sync calling on_close()')
    self:on_close()
  end
  self.state = 'CLOSED'
